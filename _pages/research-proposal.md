...
* **Markovian Smoothing:** The forward diffusion process smooths the financial trajectory, obliterating microscopic roughness ($H < 0.5$) and destroying information contained in high-frequency infinitesimal variations.
* **Counterfactual Rigidity:** The reverse diffusion process makes interactive, reactive simulation computationally expensive and prone to stochastic error accumulation over long sequences.
...
| **Autoregressive** | Superior at next-token prediction but prone to drift in long unrolls. | Evaluated as state-of-the-art in 2025 benchmarks. |
...
The **Stochastic Lévy Area**, representing the anti-symmetric part of the second-order tensor, captures asynchronous lead-lag relations:
...
3.  **Phase Transition Prediction:** FEDONet offers the first "mesh-free" early warning signal for liquidity evaporation, leveraging randomized Fourier features.
4.  **Continuous Non-Markovian Encoding:** Integration of signature-based encoders allows for history-dependent dynamics in a strictly continuous setting.
...
3.  **Generation:** Implement Neural SDEs with `torchsde` or `diffrax`, utilizing adjoint sensitivity for $O(1)$ memory.
...
## 7. Strategic Impact
* **Systematic Alpha:** Trading algorithms can be trained against a "Digital Twin" of market exchanges. The FEDONet signals the agent to stop liquidation before the book "freezes."
* **Geometric Risk Management:** We propose **Signature Expected Shortfall (S-ES)**. By quantifying path topology, residual risks are transformed into modellable "Geometric Greeks."
...
1.  Lyons, T. J. (2014). *Rough paths, Signatures and the modelling of functions on streams.*
...
4.  Cont, R., Stoikov, S., & Talreja, R. (2010). *A stochastic model for order book dynamics.* Operations Research.
5.  Smith, E., et al. (2003). *Statistical theory of the continuous double auction.* Quantitative Finance.
6.  Buccheri, G., et al. (2021). *High-Frequency Lead-Lag Effects and Cross-Asset Linkages.* JBES.
7.  Kidger, P., et al. (2021). *Neural SDEs as Infinite-Dimensional GANs.* ICML.
8.  Issa, Z., et al. (2023). *Non-adversarial training of Neural SDEs with signature kernel scores.* NeurIPS.
9.  Salvi, C., et al. (2025). *PowerSig: Efficiently computing the signature kernel.* arXiv.
10. Sojitra, A., et al. (2026). *FEDONet: Fourier-Embedded DeepONet.* JCP.
11. Nagy, P., et al. (2025). *LOB-Bench: Benchmarking Generative AI for Finance.* ICML.
12. Cont, R., & Muller, M. (2025). *A Mathematical Framework for Modeling Order Book Dynamics.* SIAM.
13. Bloch, D. (2026). *The Geometry of Risk.* arXiv.
14. Berti, L., et al. (2025). *TRADES: Generating Realistic Market Simulations.* arXiv.
15. Wang, Z., & Ventre, C. (2026). *DiffLOB: Diffusion Models for Counterfactual Generation.* arXiv.
16. Pradeleix, E., et al. (2025). *Learning non-Markovian Dynamical Systems.* ECAI.
17. Ravagnani, A., & Lillo, F. (2026). *Modeling metaorder impact with a Non-Markovian Zero Intelligence model.* Physica A.
18. Jain, K., et al. (2024). *Limit Order Book Simulations: A Review.* arXiv.
